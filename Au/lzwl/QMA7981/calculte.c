...

{
  ...
  do{
    ...
    QMA6981_GetACCData(datat);
    
    sstatex = sstatex + (unsigned long)datat[0]; // Accumulate Samples
    
    sstatey = sstatey + (unsigned long)datat[1];
...

{
  ...
  if
    (positionX[1]>=0) { //This line compares the sign of the X direction data
  ...
  if
    (positionY[1]>=0) { // Same management than in the previous case
...

{
  ...
  if
    (accelerationx[1]==0) //we count the number of acceleration samples that equals cero
  ...
  if
    (accelerationy[1]==0) //we do the same for the Y axis
...

{
  ...
  count2=0;
//  velocityx[0] = velocityx[1]=0; //Same done for the velocity variable
//  
//  velocityy[0] = velocityy[1]=0;
//  
//  positionX[0] = positionX[1]=0;
//  
//  positionY[0] = positionY[1]=0;
  do{
    
    //ADC_GetAllAxis();
    QMA6981_GetACCData(datat);
    
    accelerationx[1]=accelerationx[1] + (signed int)datat[0]; //filtering routine for noise attenuation
    
    accelerationy[1]=accelerationy[1] + (signed int)datat[1]; //64 samples are averaged. The resulting
